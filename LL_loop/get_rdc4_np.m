function [img_rebuild, split_frame, mode_frame, rdc] = get_rdc4_np(x, y, img_src, img_rebuild, split_frame, mode_frame)
    PU = 4; %4 8 16 32 64
    ...
    % loop 部分
    % blk 部分
    [prederr_blk, pred_blk, ~, mode_blk] = mode_select_blk_np(img_src, img_rebuild, x, y, PU);
    % img_rebuild_temp(x:x + PU - 1, y:y + PU - 1) = prederr_blk + pred_blk;
    img_rebuild_temp_blk = prederr_blk + pred_blk;
    mode_frame_temp_blk = fill_blk_np(mode_frame, x, y, PU, mode_blk);
    mode_bits = get_mode_bits_blk_np(0, mode_frame_temp_blk, x, y);
    rdc_curr_temp_blk = cal_rdc_np(prederr_blk, mode_bits);
    % blk 部分
    ...
    split_frame(x:x + PU - 1, y:y + PU - 1) = sp;
