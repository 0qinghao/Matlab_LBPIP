function [split_frame, mode_frame, rdc] = get_rdc32_comb(CTU, img_src, img_rebuild, split_frame, mode_frame, rdc_16)
    img_rebuild_temp = img_rebuild;
    PU = 32; %4 8 16 32 64
    PU_num = 4; %256 64 16 4 1
    find_step = 64; %1 4 16 64 256
    global zorder;
    ...
    for i = 1:PU_num
        ...
        % mode_log(i) = mode_blk;
        rdc_deep = sum(rdc_16((i - 1) * 4 + 1:(i - 1) * 4 + 4));
