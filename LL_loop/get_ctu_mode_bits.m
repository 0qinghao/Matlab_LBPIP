function sum = get_ctu_mode_bits(mode_all, size_all, maxsize, ctux, ctuy, minblk)
    % initGlobals(100);
    ...
    s = size(P);
    M = cell2mat(P');
    N = sortrows(M, 3); %根据扫描顺序排序后，按行的顺序。
    for k = 1:s(2)%先处理正方形块
        ...
        if mod(P{k}(3), 2) == 0
            ...
            if size(md) == 1%整个块只有一种模式说明是块状预测，先计算所有采用块状预测的块模式所占用比特数
                ...
                for l = 1:P{k}(3)
                    modetemp(l) = mode_all64(P{k}(1) + P{k}(3) - 1, P{k}(2) + l - 1); %看最后一行即能得到所有模式号
                end
                %计算首个模式残差存入modediff
